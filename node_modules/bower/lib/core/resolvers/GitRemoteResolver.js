...
// * If the server responds with a different assets type, the server does not support shallow cloning ("dumb server")
...
//      the assets type. This has the advantage that Git will take care of using stored credentials and any additional
...
GitRemoteResolver.prototype._supportsShallowCloning = function () {
    ...
    if (mout.string.startsWith(this._remote.protocol, 'http')
        ...
        // Calling ls-remote includes a call to the git-upload-pack service, which returns the assets type in the response.
        ...
        .spread(function (stdout, stderr) {
            // Check stderr for assets-type, ignore stdout
            var isSmartServer;

            // If the assets type is 'x-git', then the server supports shallow cloning
